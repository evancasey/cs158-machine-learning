...
class DataSet:
    ...
    def check_example(self, example):
        ...
        if self.values:
            for a in self.attrs:
            	"""print "attrs"+str(self.attrs)
            	print "values"+str(self.values)
            	print "len:"+str(len(self.values))
...
def DecisionTreeStump(dataset):
    ...
    def random_stump(dataset):
        attr = random.choice(dataset.inputs)
        stump = DecisionFork(attr, dataset.attrnames[attr])

        for (v_k, exs) in split_by(attr, dataset.examples):
            leaf = plurality_value(exs)
            stump.add(v_k, leaf)
        return stump

    def stump_learner(dataset):
        attr = choose_attribute(dataset.inputs, dataset.examples)
        stump = DecisionFork(attr, dataset.attrnames[attr])
...
def DecisionTreeLearner(dataset, stump=False):
    "[Fig. 18.5]"
